...
export default class HomePageContent extends Component {
  ...
  handleSearchDataChange = value => {
    if (this.state.searchfunction == "MatriculationNumber") {
      axios.get(`http://localhost:8002/api/skillset?matricnumber=${value}`)
        .then(response =>
          this.setState({
  ...
  handleUpdateData = () => {
    ...
    axios.get(`http://localhost:8002/api/students/?matricnumber`)
      .then(response =>
        allData.MatriculationNumber = response.data.data
      ).catch(error => console.log(error));

    axios.get(`http://localhost:8002/api/students/?studentname`)
      .then(response =>
